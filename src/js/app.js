...
async function loadRecentFiles() {
  if (!el.recentList) return; // recent files section is hidden/removed
  const files = await window.sqlBrowser.getRecentFiles();
  el.recentList.innerHTML = '';
  if (files && files.length > 0) {
    if (el.recentSection) el.recentSection.style.display = 'block';
...
function openDiscoverModal() {
  if (el.discoverModal) el.discoverModal.style.display = 'flex';
  if (el.discoverSearch) el.discoverSearch.value = '';
...
function closeDiscoverModal() {
  if (el.discoverModal) el.discoverModal.style.display = 'none';
}
...
function renderDiscoveryCards(dbs) {
  ...
  filtered.forEach(db => {
    ...
    card.addEventListener('click', () => {
      ...
      closeDiscoverModal();
      if (el.dbDropdownWrap) el.dbDropdownWrap.classList.remove('open');
    });
...
// Schema View logic moved to components/schema.js

window.buildCsv = function buildCsv(headers, rows) {
...
// Query Editor logic moved to components/query.js
...
function guessPkColumn(columns) {
  // 1. Use actual primary key if available from database schema
  const realPk = columns.find(c => c.pk === 1 || c.pk === true);
  if (realPk) return realPk.name;

  // 2. Fallbacks for safety: Try 'id', then first column with 'id', then first column
...
// ER Logic moved to components/relations.js
